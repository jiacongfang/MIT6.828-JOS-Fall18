...
sys_exofork(void)
{
	...
	struct Env *new_env;
	int error = env_alloc(&new_env, cur_id);
	if (error < 0)
		return error;
	new_env->env_status = ENV_NOT_RUNNABLE;
	new_env->env_tf = curenv->env_tf;

	// sys_exofork will appear to return 0 for the child
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	struct Env *env;
	int error_1 = envid2env(envid, &env, 1);
	if (error_1 < 0)
		return error_1;
	...
	int error_2 = page_insert(env->env_pgdir, pp, va, perm);
	if (error_2 < 0)
	{
		page_free(pp);
		return error_2;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva < (void *)UTOP)
	{
		...
		if (pp == NULL)
			return -E_INVAL;
		// (perm & PTE_W), but srcva is read-only in the
		//		current environment's address space.
		if ((*pte & PTE_W) == 0 && (perm & PTE_W) != 0)
			return -E_INVAL;

		// send page currently mapped at 'srcva'
		if ((r = page_insert((pte_t *)env->env_pgdir, pp, env->env_ipc_dstva, perm)) < 0)
			return r;
